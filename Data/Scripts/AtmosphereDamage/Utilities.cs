...
namespace BylenAtmosphericDamage
{
    public static class Utilities
    {
        ...
        public static IMySlimBlock GetBlockFromDirection(IMyCubeGrid grid, List<IMySlimBlock> blocks, Vector3D direction)
        {
            ...
            return blockPos.HasValue ? grid.GetCubeBlock(blockPos.Value) : block;
        }
        ...
        {
            return GetBlockFromDirection(grid, blocks, RandomPositionFromPoint(ref Vector3D.Zero, 1f));
        }
        ...
        public static bool TryGetEntityExposedArea(Vector3D position, Vector3D direction, BoundingBoxD bbox, IMyEntity entity, MyPlanet planet, out double areaExposed, out IMyEntity shield)
        {
            ...
            {
                areaExposed = bbox.ProjectedArea(localdir);
            }
            else
            {
